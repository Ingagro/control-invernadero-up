...
unsigned long previousMillis = 0;  // Declarar la variable para almacenar el último tiempo
...
void loop() {
  ...
  // Enviar datos solo si ha pasado el intervalo de 1 minuto
  unsigned long currentMillis = millis();
  if (currentMillis - previousMillis >= interval) {
    // Actualizar el tiempo del último envío de datos
    previousMillis = currentMillis;

    // Guardar los datos de los sensores en Adafruit IO
    temp1->save(t1);
    temp2->save(t2);
    hum1->save(h1);
    hum2->save(h2);
    ...
    // Guardar el estado de los actuadores (usando digitalRead para obtener el estado real)
    nebulizadores->save(digitalRead(NEBULIZADORES_PIN) == HIGH ? "ON" : "OFF");
    ventiladores->save(digitalRead(VENTILADORES_PIN) == HIGH ? "ON" : "OFF");
    extractor->save(digitalRead(EXTRACTOR_PIN) == HIGH ? "ON" : "OFF");
  }

  delay(1000); // Espera de 1 segundo para no sobrecargar el loop
}
